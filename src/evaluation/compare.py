"""Compare baseline, BO and MARL on identical terms — PROJECT.md section 25.
...
def kpi_table(results: dict, cfg: DictConfig) -> pd.DataFrame:
    """Build the baseline vs. BO vs. MARL KPI table — PROJECT.md section 27.2.
...
def cost_table(results: dict) -> pd.DataFrame:
    """Build the computational cost comparison — PROJECT.md section 25.2.
...
def stability_table(runs: pd.DataFrame, cfg: DictConfig) -> pd.DataFrame:
    """Summarise across seeds — PROJECT.md section 25.3.
