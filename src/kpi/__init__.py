...
All five land in ``[0, 1]``. Three are minimised;
:func:`~src.kpi.improvement.expected_rsrp_improvement` and
:func:`~src.kpi.bps.band_priority_score` are maximised, and those two are also
the ones weighted by the UE reports rather than uniformly over the grid.
Priority order is hole, overlap, expected RSRP improvement, band priority, weak
- see docs/adr/0001-five-kpis-under-lexicographic-priority.md.
"""

from src.kpi.bps import band_priority_score
from src.kpi.hole import hole_rate
from src.kpi.improvement import expected_rsrp_improvement
