"""Scene channels for a future learned radio-map predictor.

Not part of the pipeline. Only :mod:`src.surrogate.features` is kept: a digital
surface model, a signed distance field, the facade azimuth that field's gradient
carries, the depression angle from each mast to each tile, and a line-of-sight
fraction measured by casting to every sub-tile point.
...
that had to be rebuilt per evaluation would cost what it is meant to save.
